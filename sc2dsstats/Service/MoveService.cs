    ...
    public static class MoveService
    {
        ...
        public static async Task Move(Unit unit, Unit enemyinvision, Battlefield battlefield)
        {
            ...
            if (enemyinvision.Name != null)
            {
                ...
                speed = unit.Speed / StartUp.Battlefieldmodifier;
                size = enemyinvision.Size / StartUp.Battlefieldmodifier;
                //speed = unit.Speed;
                if (speed < 1)
                    speed = 1;
                if (DEBUG) _logger.LogDebug(unit.ID + " moving to enemy " + enemyinvision.ID + " with speed " + speed);
            }
            else if (unit.Owner <= 3)
            {
                ...
                if (DEBUG) _logger.LogDebug(unit.ID + " moving to def2");
            }
            else if (unit.Owner > 3)
            {
                ...
                if (DEBUG) _logger.LogDebug(unit.ID + " moving to def1");
            }
            ...
            if (hasTarget == true)
            {
                ...
                if (DEBUG) _logger.LogDebug(unit.ID + " Moving to " + newpos + " (" + i + ")");
                unit.Pos = newintpos;
                unit.RealPos = newpos;
                ...
                if (DEBUG) _logger.LogDebug(unit.ID + " stuck");
