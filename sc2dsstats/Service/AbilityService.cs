...
namespace paxgame3.Client.Service
{
    public class AbilityService
    {
                ...
                if (abilities.Count() > 0)
                {
                    foreach (var ability in abilities)
                    {
                        if (ability.Cost == 0 || unit.Ownerplayer.AbilityUpgrades.SingleOrDefault(x => x.Ability == ability.Ability) != null)
                        {
                            ...
                            {
                                if (DEBUG) _logger.LogDebug(unit.ID + " Activating ability " + ability.Ability);
        ...
        public static async Task UseAbilities(Unit unit, Battlefield battlefiled, List<Unit> enemies, List<Unit> allies)
        {
            ...
            {
                if (DEBUG) _logger.LogDebug(unit.ID + " Removing ability " + ability.Ability + " " + unit.Name);
            ...
            {
                if (DEBUG) _logger.LogDebug(unit.ID + " regenerating shield");
            ...
            {
                if (DEBUG) _logger.LogDebug(unit.ID + " regenerating hp");
